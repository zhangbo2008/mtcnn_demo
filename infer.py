...
class Detector:
    def __init__(self, pnet_path, rnet_path, onet_path, softnms=False, thresholds=None, factor=0.709):
        ...
        self.pnet.load_state_dict(torch.load(pnet_path,map_location=device))
        self.rnet.load_state_dict(torch.load(rnet_path,map_location=device))
        self.onet.load_state_dict(torch.load(onet_path,map_location=device))
...
if __name__ == '__main__':
    img_path = r"data/detect_img/10.jpg"
    img = Image.open(img_path)
    # img_draw = ImageDraw.Draw(img)
    model_use='param/'
    detector = Detector(model_use+"p_net.pth", model_use+"r_net.pth", model_use+"o_net.pth")
    ...
    cv2.imwrite(os.path.join("out_img111111111", img_path), img)
